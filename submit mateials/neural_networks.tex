% Meta-LF-PID Network Architecture - PlotNeuralNet Style
% 需要安装: pdflatex + tikz
% 编译: pdflatex neural_networks.tex
% 输出: neural_networks.pdf → 重命名为 neutral_network.pdf
...
% Title
%============================================================================
\node[font=\rmfamily\Large\bfseries, text=textcolor] at (8, 7.5) 
    {Meta-LF-PID Network Architecture};
\node[font=\rmfamily\normalsize, text=gray] at (8, 6.8)
...
% 输入特征标注（左侧 - 加大字体）
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 3.2) 
    {Mass ($m$)};
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 2.7) 
    {DOF ($n$)};
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 2.2) 
    {Link Lengths ($L_i$)};
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 1.7) 
    {Inertia ($I_i$)};
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 1.2) 
    {COM ($r_i$)};
\node[anchor=east, font=\rmfamily\footnotesize, text width=2.5cm, align=right] at (-0.3, 0.7) 
    {Friction ($\mu_i$), \dots};
...
% 权重矩阵标注
\node[font=\rmfamily\scriptsize, text=yellow!90, align=center] at (3.35, 0.8)
    {\textbf{$W_1$: 10×256}};
...
% 权重矩阵标注（与Encoder 1不同）
\node[font=\rmfamily\scriptsize, text=yellow!90, align=center] at (6.40, 0.8)
    {\textbf{$W_2$: 256×256}};
...
\node[label=white, font=\rmfamily\footnotesize\bfseries] at (12.1, 4.5) {Base Gains};
\node[dimension, text=white] at (12, 4.05) {$\bar{\bm{K}}$};
\node[dimension, text=white] at (12, 3.7) {$3n$};

% Input Scales Head
\drawcube{11.5}{1.5}{1}{1.5}{0.8}{outputcolor}
\node[label=white, font=\rmfamily\footnotesize\bfseries] at (12.1, 2.5) {Scales};
\node[dimension, text=white] at (12, 2.05) {$\bm{s}$};
\node[dimension, text=white] at (12, 1.7) {$3n$};

% TS Consequents Head
\drawcube{11.5}{-0.5}{1}{1.5}{0.8}{outputcolor}
\node[label=white, font=\rmfamily\footnotesize\bfseries] at (12.1, 0.5) {TS Conseq.};
\node[dimension, text=white] at (12, 0.05) {$\bm{c}$};
\node[dimension, text=white] at (12, -0.3) {$d_c$};

% 从Hidden到三个Head的连接
\draw[arrow, hiddencolor!70!black] (9.8, 2.2) -- (11.3, 4.3);
\draw[arrow, hiddencolor!70!black] (9.8, 2.0) -- (11.3, 2.3);
\draw[arrow, hiddencolor!70!black] (9.8, 1.8) -- (11.3, 0.3);

%============================================================================
% Sigmoid激活函数
%============================================================================
\node[activation=activationcolor, font=\Large, text=white] at (13.5, 4.3) {$\sigma$};
\node[activation=activationcolor, font=\Large, text=white] at (13.5, 2.3) {$\sigma$};
\node[activation=activationcolor, font=\Large, text=white] at (13.5, 0.3) {$\sigma$};

% 连接
\draw[arrow, outputcolor!70!black] (12.7, 4.3) -- (13, 4.3);
\draw[arrow, outputcolor!70!black] (12.7, 2.3) -- (13, 2.3);
\draw[arrow, outputcolor!70!black] (12.7, 0.3) -- (13, 0.3);

%============================================================================
% 最终输出标注（K符号向下移一点）
%============================================================================
\node[anchor=west, font=\rmfamily\small] at (14.1, 4.3) 
    {$\hat{\bar{\bm{K}}} \in [0,1]^{3n}$};
\node[anchor=west, font=\rmfamily\small] at (14.1, 2.3)
    {$\hat{\bm{s}} \in [0,1]^{3n}$};
\node[anchor=west, font=\rmfamily\small] at (14.1, 0.3)
    {$\hat{\bm{c}} \in [0,1]^{d_c}$};
      ...
      font=\rmfamily\normalsize, drop shadow] at (8.5, -2.2)
{
    ...
    $\displaystyle \mathcal{L}_{meta} = \frac{1}{N}\sum_{v=1}^{N} w_v\left\|\bm{\theta}_v^* - \hat{\bm{\theta}}_v\right\|_2^2$\\[0.12cm]
    \footnotesize where $\bm{\theta}_v^* = [\bar{\bm{K}}_v^*, \bm{s}_v^*, \bm{c}_v^*]$ are ground-truth LF-PID parameters\\
    \footnotesize from hybrid optimization (DE + Nelder--Mead), $N=232$ filtered virtual robots
};
...
\node[draw, thick, rounded corners, fill=green!15, text width=3.5cm, align=left,
      font=\rmfamily\footnotesize, drop shadow] at (3.5, -5.5)
{
    ...
    • Output: $\hat{\bm{\theta}} \in [0,1]^D$\\[0.05cm]
    \footnotesize $D\!=\!330n$ (2970 for 9-DOF)\\[0.05cm]
    \textbf{Total: $\sim\!10^5$ params}
};

% 中间：训练统计
\node[draw, thick, rounded corners, fill=blue!15, text width=3.2cm, align=left,
      font=\rmfamily\footnotesize, drop shadow] at (8.7, -5.5)
{
    \textbf{Training Statistics:}\\[0.1cm]
    • Samples: 232 (filtered)\\
    • Epochs: 1000\\
    • Optimizer: AdamW\\
    • LR: $10^{-3}$\\
    • Time: 8 minutes\\
...
\node[draw, thick, rounded corners, fill=orange!15, text width=3.2cm, align=left,
      font=\rmfamily\footnotesize, drop shadow] at (13.5, -5.5)
{
    \textbf{Key Innovations:}\\[0.1cm]
    • Hierarchical Design\\
    • LayerNorm Stability\\
    • Structured Param Heads\\
    \footnotesize\quad ($\bar{\bm{K}}$, $\bm{s}$, $\bm{c}$)\\
    \footnotesize • Sigmoid Bounded\\
    • Fast Inference\\
      ...
      font=\rmfamily\scriptsize, drop shadow] at (-2.3, 7)
{
    ...
    \footnotesize 303 $\rightarrow$ \textbf{232 Filtered}
};
...
\node[draw, thick, rounded corners, fill=orange!10, text width=4.5cm, align=left,
      font=\rmfamily\footnotesize, drop shadow] (encoder_explanation) at (-1.0, -2.5)
{
